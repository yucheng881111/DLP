...
from utils import init_weights, kl_criterion, finn_eval_seq, kl_criterion_lp
...
class kl_annealing():
    def __init__(self, args):
        super().__init__()
        self.use_cyc = args.kl_anneal_cyclical
        self.ratio = args.kl_anneal_ratio
        self.num_cyc = args.kl_anneal_cycle
        self.weight = 0
        self.total_epoch = args.niter
    
    def update(self, epoch):
        if self.use_cyc:
            one_cyc = self.total_epoch // self.num_cyc
            e = epoch % one_cyc
            if e < one_cyc / 2:
                self.weight = (1 / (one_cyc / 2)) * e
            else:
                self.weight = 1

        else:
            if epoch < self.total_epoch / 2:
                self.weight = (1 / (self.total_epoch / 2)) * epoch
            else:
                self.weight = 1
...
def main():
    ...
    if args.model_dir != '':
        ...
        decoder = vgg_decoder(args.g_dim + 128) # cat condition
    ...
    for epoch in range(start_epoch, start_epoch + niter):
        ...
        with open('record.txt', 'a') as f:
            f.write('epoch ' + str(epoch) + ':  kl: ' + str(kl_anneal.get_beta()) + '  tfr: ' + str(args.tfr))
            f.write('\n')
        '''
        ...
        for _ in range(args.epoch_size):
            ...
            cond = [x.to(device) for x in cond]
        ...
        if epoch % 5 == 0:
            ...
            for _ in range(88):
                try:
                    validate_seq, validate_cond, _ = next(validate_iterator)
                except StopIteration:
                    validate_iterator = iter(validate_loader)
                    validate_seq, validate_cond, _ = next(validate_iterator)

                validate_seq = [x.to(device) for x in validate_seq]
                validate_cond = [x.to(device) for x in validate_cond]
