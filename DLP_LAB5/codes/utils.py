...
def kl_criterion_lp(mu, logvar, mu_p, logvar_p, args):
    sigma1 = logvar.mul(0.5).exp()
    sigma2 = logvar_p.mul(0.5).exp()
    kld = torch.log(sigma2/sigma1) + (torch.exp(logvar) + (mu - mu_p)**2)/(2*torch.exp(logvar_p)) - 1/2
